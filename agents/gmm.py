...
class MixtureGaussianPolicy(nn.Module):
    def __init__(self, state_dim, action_dim, num_mixtures=2, sample_quantile=0.6):
        ...
        self.first_layer = nn.Sequential(nn.Linear(state_dim, 1024), nn.ReLU())
        self.second_layer = nn.Sequential(nn.Linear(1024, 1024), nn.ReLU())

        self.mean_layer = nn.Linear(1024, action_dim * num_mixtures)
        self.log_std_layer = nn.Linear(1024, action_dim * num_mixtures)
        self.weights_layer = nn.Linear(1024, num_mixtures)
    ...
    def forward(self, state):
        ...
        logits = self.weights_layer(x)
        #logits = torch.clamp(logits, -10, -2)
...
class GMM(BaseAgent):
    ...
    def get_action(self, state):
        ...
        action_mean, action_std = self.policy.sample_action(state_prep)
        clipped_action = np.clip(action_mean.cpu().numpy(), -1, 1)
